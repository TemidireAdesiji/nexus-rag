  ...
  return (
    ...
    >
      {allMessages.map((msg, idx) => {
        ...
        const isCurrentStreaming = isLast && streamingMessage !== null && !isUser;
          ...
          >
            ...
            >
              ...
              {msg.sources && msg.sources.length > 0 && (
                <Stack spacing={1} sx={{ mt: 1.5 }}>
                  ...
                  {msg.sources.map((src, sIdx) => (
                    <SourceCard key={`${src.origin}-${sIdx}`} source={src} index={sIdx} />
                  ))}
                </Stack>
              )}
              ...
              <Typography
                variant="caption"
                color="text.disabled"
                sx={{ display: "block", mt: 0.75, textAlign: isUser ? "right" : "left" }}
              >
      ...
      {loading && !streamingMessage && (
        <Box sx={{ display: "flex", gap: 1.5, mb: 2.5, alignItems: "flex-start" }}>
          <Avatar sx={{ width: 34, height: 34, bgcolor: "secondary.main", mt: 0.5 }}>
            <SmartToyOutlinedIcon fontSize="small" />
          </Avatar>
          <Paper
            elevation={0}
