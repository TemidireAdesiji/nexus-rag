 ...
 */
router.get("/api/engagements", async (req: RequestWithId, res: Response, next: NextFunction) => {
  try {
    const nameFilter = req.query.name as string | undefined;
    const query = nameFilter ? { participants: { $regex: nameFilter, $options: "i" } } : {};

    const engagements = await Consultation.find(query).sort({ scheduledDate: -1 }).lean();
