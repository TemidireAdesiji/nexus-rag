  ...
  async (req: RequestWithId, res: Response, next: NextFunction) => {
    try {
      ...
      const query = companyFilter
        ? { companyName: { $regex: companyFilter, $options: "i" } }
        : {};
  ...
  async (req: RequestWithId, res: Response, next: NextFunction) => {
    try {
      ...
      for (const holding of holdings) {
        ...
        statusBreakdown[holding.currentStatus] =
          (statusBreakdown[holding.currentStatus] || 0) + 1;
      ...
      const analysis: PortfolioAnalysis = {
        ...
        averageInvestmentSize:
          holdings.length > 0 ? totalCapitalDeployed / holdings.length : 0,
        statusBreakdown,
